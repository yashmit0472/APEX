...
contract PaymentEscrowFuzzTest is Test {
    ...
    function testFuzzSettledJobCannotBeSettled(
        bytes32 hash
    ) public {
    ...
    function testFuzzRefundedJobCannotBeRefunded(
        uint256 extraTime
    ) public {
    ...
    function testFuzzRefundedJobCannotBeSettled()
        public
    {
        vm.prank(vault);
        uint256 jobId = escrow.createJob(
    ...
    function testFuzzDeliveryOnlyOnce(
        bytes32 hash1,
        bytes32 hash2
    ) public {
    ...
    function testFuzzOnlyAssignedProviderDelivers(
        address caller
    ) public {
    ...
    function testFuzzLockedNeverExceedsTotal(
        uint8 jobCount,
        uint256 seed
    ) public {
    ...
    function testFuzzEscrowSolvency(
        uint8 jobCount
    ) public {
    ...
    function testFuzzStakeLockedUntilTerminal()
        public
    {
        vm.prank(vault);
        uint256 jobId = escrow.createJob(
        ...
        assertEq(
            stakeManager.lockedBalance(provider),
            lockedAfterCreate - 10 * ONE_USDC
        );
    }

    // ── Fuzz: random amounts settle correctly ──

    function testFuzzSettleRandomAmounts(
        uint256 payment,
        uint256 stake
    ) public {
        payment = bound(
    ...
    function testFuzzRefundRandomAmounts(
        uint256 payment,
        uint256 stake
    ) public {
        payment = bound(
